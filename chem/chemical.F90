...
! PUBLIC DERIVED TYPES: 
   type,extends(type_base_model),public :: type_ulg_chemical 
      type (type_state_variable_id)         :: id_dox,id_nhs,id_nos,id_odu,id_dic,id_pho
...
!     Model parameters 
   ...
   call self%register_state_variable(self%id_pho, 'PHO', 'mmol C m-3', 'Dissolved phosphorus concentration', minimum=0.0e-7_rk)
   ...
   call self%register_diagnostic_variable(self%id_anammox, 'anammox', '-','-', output=output_instantaneous) 
   call self%register_diagnostic_variable(self%id_nitrification, 'nitrification', 'mmol N m-3 d-1','Nitrification', output=output_instantaneous) 
   call self%register_diagnostic_variable(self%id_oxidation_nitrate, 'oxidation_nitrate', 'mmol N m-3 d-1','Oxudation by nitrate', output=output_instantaneous) 
   call self%register_diagnostic_variable(self%id_oxidation_oxygen, 'oxidation_oxygen', 'mmol O m-3 d-1','Oxydation by oxygen', output=output_instantaneous) 
   return 
...
99 call self%fatal_error('Chemical', 'Error reading namelist ulg_chemical') 
   ...
   _DECLARE_ARGUMENTS_DO_
      ...
      real(rk) ::  DOX,NHS,NOS,ODU,DIC,PHO
   ...
   _GET_(self%id_pho,PHO)       ! Dissolved inorganic carbon concentration
   ...
   ! Compute Ammonium oxidation by nitrate 
    Rate_oxidation_NHS_NOS = tf * self%rox_nhs_nos * Inhibition(ODU,self%ki_nhs_odu) * Inhibition(DOX,self%ki_nhs_o2) * Michaelis(NOS,0.3_rk)
   ...
   _ADD_SOURCE_(self%id_dic, 0.0_rk)
  ...
  real                 :: temp,wnd,salt
  real                 :: dox, rho, wind
  real,parameter       :: secs_pr_day=86400.0_rk
  real                 :: Dox_sat, Piston
   ...
   _ADD_SURFACE_FLUX_(self%id_dox, (Dox_sat - dox) * Piston )
  ...
  real, intent(in) :: t, w ! temp, wind
  real :: schmidt      ! The Schmidt number
  ...
  piston_wanninkhof = 0.266*w*w/SQRT(schmidt/600.) / 360000.
   ...
   ! Weiss formula
  real, intent(in) :: t, s, rho	! temp, salt, dens
  real         :: tk, sat_ox
  real, parameter ::  a1 = -173.9894,      &
       a2 = 255.5907,       &
       a3 = 146.4813,       &
       a4 = -22.204,        &
       b1 = -0.037362,      &
