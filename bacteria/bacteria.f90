...
!     Model parameters 
   ...
   call self%add_to_aggregate_variable(standard_variables%total_phosphorus, self%id_bac, scale_factor=self%NCrBAC*self%r_p_n_redfield)

    ! Register diagnostic variables 
   call self%register_diagnostic_variable(self%id_remineralization_anoxic_bac, 'remineralization_anoxic_bac', '-', & 
      '-', output=output_instantaneous) 
   call self%register_diagnostic_variable(self%id_oxygen_consumption_bac, 'oxygen_consumption_bac', '-', & 
      '-', output=output_instantaneous) 
   call self%register_diagnostic_variable(self%id_respiration_bac, 'respiration_bac', 'mmol C m-3 d-1', & 
   ...
   call self%register_diagnostic_variable(self%id_uptake_dcl_bac, 'uptake_dcl_bac', 'mmol C m-3 d-1', & 
...
99 call self%fatal_error('Bacteria', 'Error reading namelist ulg_bacteria') 
   ...
   _DECLARE_ARGUMENTS_DO_
      ...
      real(rk) ::  temp
      real(rk) ::  BAC
      real(rk) ::   Denitrificaiton	  ! mmol N m-3, Denitrification flux
   ...
   ! Test if bacterial growth is limited by carbon (DOClabile) or nitrogen (ammonium + DONLabile) 
    testratio = Uptake_DCL * (Uptake_DNL/Uptake_DOCL_local - self%eff_gr_bac_c*self%r_n_c_bac)
