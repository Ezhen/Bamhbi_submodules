...
!     Model parameters 
   ...
   call self%register_state_variable(self%id_dns, 'DNS', 'mmol N m-3', 'Semi-labile detritus concentration in nitrogen',minimum=0.0e-7_rk, vertical_movement=self%w_dom) 
...
99 call self%fatal_error('DOM', 'Error reading namelist ulg_dom') 
   ...
   ! Compute hydrolysis rates of semi-labile detritus 
    Hydrolysis_DSC_DCL = tf * self%hmax_dsl  * Michaelis(DCS,self%ks_dsc_bac) * BAC
    Hydrolysis_DNS_DNL = tf * self%hmax_dsl  * Ratio(DNS,DCS) * Michaelis(DCS,self%ks_dsc_bac) * BAC
