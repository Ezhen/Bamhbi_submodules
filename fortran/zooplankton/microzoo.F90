...
! PUBLIC DERIVED TYPES: 
   type,extends(type_base_model),public :: type_ulg_microzoo 
      ...
      type (type_state_variable_id)         :: id_bac,id_cdi,id_cem,id_cfl,id_dcl,id_dcs,id_dic,id_dnl,id_dns,id_dox,id_mes,id_ndi,id_nem,id_nfl,id_nhs,id_pho,id_poc,id_pon,id_sid
...
!     Model parameters 
   ...
   call self%register_state_variable(self%id_mic, 'MIC', 'mmol C m-3', 'Microzooplakton biomass', minimum=0.0e-7_rk) 
   ...
   call self%add_to_aggregate_variable(standard_variables%total_nitrogen, self%id_mic, scale_factor=self%r_n_c_mic)
   call self%add_to_aggregate_variable(standard_variables%total_phosphorus, self%id_mic, scale_factor=self%r_n_c_mic*self%r_p_n_redfield)
...
99 call self%fatal_error('MicroZoo', 'Error reading namelist ulg_microzoo') 
   ...
   _DECLARE_ARGUMENTS_DO_
      ...
      real(rk) ::  BAC,CDI,CEM,CFL,DCL,DCS,DIC,DNL,DNS,DOX,MES,NDI,NEM,NFL,NHS,PHO,POC,PON,SID
      real(rk) ::  temp
      real(rk) ::  MIC
   ...
   _GET_(self%id_mes,MES)       ! Dissolved oxygen concentration
   ...
   _ADD_SOURCE_(self%id_mes, -Grazing_C/Prey_C*self%eff_mic_mes*MES)
