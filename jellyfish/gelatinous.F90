...
!     Model parameters 
      ...
      real(rk)     :: mo_anox_pred, momax_gel, q10_gel, r_n_c_gel, r_n_c_mes
...
!     Model parameters 
   ...
   call self%get_parameter(self%t_g_gel, 't_g_gel', 'mmolC m-3', 'Feeding threshold for GEL grazing', default=0.0_rk) 
   ...
   call self%add_to_aggregate_variable(standard_variables%total_nitrogen, self%id_gel, scale_factor=self%r_n_c_gel)
   call self%add_to_aggregate_variable(standard_variables%total_phosphorus, self%id_gel, scale_factor=self%r_n_c_gel*self%r_p_n_redfield)
...
99 call self%fatal_error('Gelatinous', 'Error reading namelist ulg_gelatinous') 
   ...
   ! Egestion rate of zooplankton 
    Egestion_C = zoo_egestion(self%eff_ass_gel_prey,Grazing_C)
   ...
   _ADD_SOURCE_(self%id_poc, Egestion_C + Mortality_C - Ratio(Grazing_C,Prey_C) * self%eff_gel_pom*POC) 
   _ADD_SOURCE_(self%id_pon, Egestion_N + Mortality_N - Ratio(Grazing_C,Prey_C) * self%eff_gel_pom*PON) 
   _ADD_SOURCE_(self%id_nhs, Excretion_N_adj) 
   _ADD_SOURCE_(self%id_pho, Excretion_N_adj * self%r_p_n_redfield)
   _ADD_SOURCE_(self%id_cfl, -Ratio(Grazing_C,Prey_C) * self%eff_gel_fla * CFL) 
   _ADD_SOURCE_(self%id_cem, -Ratio(Grazing_C,Prey_C) * self%eff_gel_emi * CEM)
   _ADD_SOURCE_(self%id_cdi, -Ratio(Grazing_C,Prey_C) * self%eff_gel_dia * CDI)
   _ADD_SOURCE_(self%id_nfl, -Ratio(Grazing_C,Prey_C) * self%eff_gel_fla * NFL)
   _ADD_SOURCE_(self%id_nem, -Ratio(Grazing_C,Prey_C) * self%eff_gel_emi * NEM)
   _ADD_SOURCE_(self%id_ndi,- Ratio(Grazing_C,Prey_C) * self%eff_gel_dia * NDI) 
   _ADD_SOURCE_(self%id_mic, -Ratio(Grazing_C,Prey_C) * self%eff_gel_mic * MIC) 
   _ADD_SOURCE_(self%id_mes, -Ratio(Grazing_C,Prey_C) * self%eff_gel_mes * MES)
   _ADD_SOURCE_(self%id_sid, Ratio(Grazing_C,Prey_C) * self%eff_gel_dia * NDI * self%r_si_n_dia)
