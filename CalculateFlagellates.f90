...
99 call self%fatal_error('Flagellates', 'Error reading namelist ulg_Flagellates') 
   ...
   ! Calculate ratios in phytoplankton 
    ...
    Ratio_Chl_C = ChlCrPHY(Ratio_N_C,self%rmax_n_c_fla,self%rmin_n_c_fla,self%rmin_chl_n_fla,self%rmax_chl_n_fla)
    
   ! chlorophyll(i,j,k,1) = ChlCrFlagellates * CFL ! REMOVE (POSSIBLY) 
    
   ! Nitrate uptake rate 
    Uptake_NO3 = NOuptake(Ratio_N_C,tf,self%rmax_n_c_fla,self%umax_nos_fla) * Michaelis(NOS,self%ks_nos_fla) * Inhibition(NHS,self%ki_nhs_phy) * CFL
    
   ! Ammonium uptake rate 
    Uptake_NHS = NUTuptake(Ratio_N_C,(NHS-0.0),tf,self%rmax_n_c_fla,self%umax_nhs_fla,self%ks_nhs_fla) * CFL
    
   ! Phosphate uptake rate 
    Uptake_PO4 = NUTuptake(Ratio_N_C,(PHO-0.0),tf,self%rmax_n_c_fla,self%umax_po4_fla,self%ks_po4_fla) * CFL
   ...
   ! Compute actual N:C and Si:C ratios 
    Ratio_N_C = Ratio_PHYT(Ratio_N_C,self%rmax_n_c_fla,self%rmin_n_c_fla)
    Ratio_Si_C = Ratio_PHYT(1.0,1.0,0.0)
    
   ! Compute nutrient and light limitation 
    Limitation_nutrient = Nutr_LIM(self%rmin_n_c_fla,0.0,Ratio_N_C,Ratio_Si_C)
    Limitation_light = 1.-exp(-self%pi_fla * par / self%mumax_fla)
    
   ! Compute carbon uptake 
    Uptake_C = self%mumax_fla * Limitation_light * Limitation_nutrient * CFL * tf
   ...
   ! Compute the extra DOC excretion from the difference of growth in nutrient limited (actual NC ratio) and nutrient saturated (max NC ratio) 
    Excretion_ext = CFL * tf * self%exc_extra_doc * self%mumax_fla * ExtraEXCR_term(Limitation_light,self%rmin_n_c_fla,self%rmax_n_c_fla,Ratio_N_C)
   ...
   ! Compute mortality 
    Mortality_C  = PHYMORT(self%mo_fla,tf) * CFL
    Mortality_N  = PHYMORT(self%mo_fla,tf) * NFL
