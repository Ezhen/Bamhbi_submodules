...
99 call self%fatal_error('Emiliana', 'Error reading namelist ulg_emiliana') 
   ...
   ! Ammonium uptake rate 
    Uptake_NHS = uptake_nutrient_phyt(Ratio_N_C,(NHS-0.0),tf,self%rmax_n_c_emi,self%umax_nhs_emi,self%ks_nhs_emi) * CEM
    
   ! Phosphate uptake rate 
    Uptake_PO4 = uptake_nutrient_phyt(Ratio_N_C,(PHO-0.0),tf,self%rmax_n_c_emi,self%umax_po4_emi,self%ks_po4_emi) * CEM
   ...
   ! Compute nutrient and light limitation 
    Limitation_nutrient = limitation_by_nutrient(self%rmin_n_c_emi,0.0,Ratio_N_C,Ratio_Si_C)
