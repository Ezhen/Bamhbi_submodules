...
99 call self%fatal_error('Flagellates', 'Error reading namelist ulg_flagellates') 
   ...
   ! Ammonium uptake rate 
    Uptake_NHS = uptake_nutrient_phyt(Ratio_N_C,(NHS-0.0),tf,self%rmax_n_c_fla,self%umax_nhs_fla,self%ks_nhs_fla) * CFL
    
   ! Phosphate uptake rate 
    Uptake_PO4 = uptake_nutrient_phyt(Ratio_N_C,(PHO-0.0),tf,self%rmax_n_c_fla,self%umax_po4_fla,self%ks_po4_fla) * CFL
   ...
   ! Compute actual N:C and Si:C ratios 
    ...
    Ratio_Si_C = ratio_adjustment(1.0,1.0,0.0)
    
   ! Compute nutrient and light limitation 
    Limitation_nutrient = limitation_by_nutrient(self%rmin_n_c_fla,0.0,Ratio_N_C,Ratio_Si_C)
